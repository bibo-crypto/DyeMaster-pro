...
class Validators:
    ...
    @staticmethod
    def validate_color_object(data: dict) -> Tuple[bool, str, dict]:
        """التحقق من صحة كائن اللون بالكامل"""
        cleaned_data = data.copy()
        
        # التحقق من الكود
        is_valid, msg = Validators.validate_color_code(data.get('code', ''))
        if not is_valid: return False, msg, {}
        
        # التحقق من الاسم
        is_valid, msg = Validators.validate_name(data.get('name', ''), "Color Name")
        if not is_valid: return False, msg, {}
        
        # التحقق من السعر
        is_valid, msg, price = Validators.validate_price(data.get('price_kg', 0))
        if not is_valid: return False, msg, {}
        cleaned_data['price_kg'] = price
        
        # التحقق من النسبة
        is_valid, msg, resa = Validators.validate_percentage(data.get('resa_percent', 0), "Resa %", allow_zero=True)
        if not is_valid: return False, msg, {}
    ...
    def validate_color_object(color_data: dict, allowed_dye_types: list = None) -> Tuple[bool, str, Optional[dict]]:
        ...
        is_valid, message, resa = Validators.validate_percentage(
            cleaned_data.get('resa_percent', 0), "RESA percentage", min_value=0, max_value=100, allow_zero=True
        )
        if not is_valid:
